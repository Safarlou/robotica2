...
Practically all code and algorithms that are not discussed in this section were devised and programmed by us.
...
In the final version of our program, the path calculated moves between the edges of navmesh areas -- specifically the centres of these edges -- as can be seen by comparing \fref{fig:consolidatedTriangles} and \fref{fig:computedPath}. This means that the path found is hardly ever optimal: it runs through the middle of areas and passages instead of sticking to walls and cutting corners, as a human might.

We did devise and program an algorithm to refine the path once it was generated, such that it became more optimal. The algorithm worked just fine in almost all situations, but there was a single situation that could occur in which the refinement caused the robot to get stuck. Since we only discovered this logical error very near the end of the project, we were not able to fix the algorithm in time, and were forced to disable path refinement.

Path refinement would not be a very difficult problem if not for the need to account for the size of the robot. If we could simply find a path between corners on walls, no path refinement would be necessary. However, the robot has a width and cannot move directly up to a wall - the path needs to ensure it remains at some distance from the wall.

The algorithm relies on two basic principles. The first in short is that, given three subsequent points on the existing path, if the last point can be reached from the first point by a straight line without hitting obstacles, the middle point can be removed. The second principle is based on avoiding the sides of obstacles by making use of vector calculations. Both principles are demonstrated in \fref{fig:refinementPrinciples}. The combination of these leads to satisfactory results in almost all case, as noted. A result of the algorithm can be seen in \fref{fig:refinedpathResult}.

\begin{figure}[h]
        \centering
        \begin{subfigure}[t]{0.45\columnwidth}
        \centering
		\input{pictures/refinementStraightLines.tex}
		\subcaption{\small Although initial pathing is between navmesh edge centres, the cyan line demonstrates that the middle path point can be removed without running into obstacles.}
        \end{subfigure}
        \begin{subfigure}[t]{0.45\columnwidth}
        \centering
		\input{pictures/refinementObstacleDistance.tex}
		\subcaption{\small The cyan refined path shows how the path is taking sharp corners, while taking the size of the vehicle into account (dashed lines)}
        \end{subfigure}
        \caption{\small Edge-length method of avoiding small passages.}
        \label{fig:refinementPrinciples}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth]{pictures/refinedpathResult.png}
	\caption{\small A result of the existing path refinement algorithm, used on a mock-up image (the red walls were added into a photo). Note that the path, in white, takes optimzed corners.}
	\label{fig:refinedpathResult}
\end{figure}
...
We did work around this by replacing our large transport robot with the smaller guard robot, but this was a last minute decision. In the event where we actually would have had two robots, it would have been more practical to have two small robots rather than a large and a small one.
...
Chris & Object construction, bluetooth research, presentation \\ 
Erik & Sockets research, MRDS research, initial project outline, peer review \\ 
...
When we started out with the project we had a great outline. We had a basic plan which we could extend if we had time left. Everybody could get started on their own part of the project and get modular working parts we could fit when it all came together. What we missed was a method of project management. I agree with Erik a lot, the integration between our parts and the real world went rough. The workload wasn’t evenly distributed and a lot of work was done that wasn’t implemented in the final project in the end. 
I’m very satisfied with what we managed to deliver, but it was mostly not my work. I learned a lot about the importance of team management after working a lot on the robots design and designing the jobs that we wanted the robots to do, while we couldn’t finish all these tasks. Even though I learned a lot about designing projects, we didn’t bring it to practice. It would have gone a lot better if we had chosen some kind of project management for our project.
...
I think the biggest problem we really had was project management and communication. For varying reasons, no-one stepped up to lead the project or to coordinate our efforts. This led to poor communication, which led to an uneven distribution of the workload and tensions, despite the fact that every member had a sincere desire to make the project a success. I've given this quite a bit of thought, but am not yet sure how I would solve this in the future. 
...
Although I am very satisfied with what we managed to achieve, I am disappointed with how we worked as a team.

In my experience, several members were not able to keep clear agreements, and at crucial times displayed minimal involvement with the project. Because of this, time and again two members felt forced to take others' tasks upon themselves, lest they not be completed on schedule.

There are several things I myself could have done to change our course, which I will be sure to apply in future team projects. First, there should be frequent meetings to discuss everyone's progress on tasks. Second, when distributing tasks, deadlines should be set for completing them. Third, a team leader should be appointed, who would be responsible for ensuring deadlines are met.

Apart from team management, I feel that I learned many things about programming, maths, geometry, and other subjects. I improved my skills in C\#, Visual Studio, Git and \LaTeX. Especially, I learned a lot about computer vision and pathfinding.
...
After demo day, we made several small changes to the code that mainly concern comments and source citing. To view the older version of the code that was ran on the actual day, see \link{https://github.com/MareinK/robotica2/tree/f4c68dcab28f35ed3ac389d2fb01d46ae9705ea7/Code/v1/Main}{here}.

Git keeps a complete version history that may give interesting insights into how the project developed. Since we started writing this report (which is also stored in the repository), the commit history has been getting a bit messy. Therefore we suggest looking at the history of actual programming work up to January 16th (demo day), found \link{https://github.com/MareinK/robotica2/commits/master?page=3}{here}.

\section{Miscellaneous Material}

\begin{description}
	\item[\link{http://prezi.com/hay_qlcqdzkp/convoi}{Presentation slides}]
	\item[\link{http://www.youtube.com/watch?v=vSR03ovDv_w}{Presentation and demonstration video}]
